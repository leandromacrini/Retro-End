...
InputConfigModel::InputConfigModel(int deviceId) : mDeviceId(deviceId)
{
	mPlayerNum = -1;
}

void InputConfigModel::clear()
{
	mNameMap.clear();
}

void InputConfigModel::mapInput(const string& name, Input input)
{
	mNameMap[toLower(name)] = input;
}

Input InputConfigModel::getInputByName(const string& name)
{
	return mNameMap[toLower(name)];
}

bool InputConfigModel::isMappedTo(const string& name, Input input)
{
...
vector<string> InputConfigModel::getMappedTo(Input input)
{
	vector<string> maps;
...
void InputConfigModel::loadFromXML(pugi::xml_node node, int playerNum)
{
	this->clear();
...
void InputConfigModel::writeToXML(pugi::xml_node parent)
{
...
void InputConfigModel::setPlayerNum(int num) { mPlayerNum = num; }
int InputConfigModel::getPlayerNum() { return mPlayerNum; }
int InputConfigModel::getDeviceId() { return mDeviceId; }
